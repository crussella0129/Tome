...
| [INT-0016](../../../intents/INT-0016-showcase-readme.md) | 4 — reproducible generators; installable mono TTF; no committed font binaries | T-040/T-041/T-043 | generators rerun clean; `make-terminal-font.py` emits a valid monospace TTF (installed); `git check-ignore` confirms no `.ttf`/`.woff2` tracked |

## Unit Tests
- **Intent:** [INT-0016](../../../intents/INT-0016-showcase-readme.md)
- `test_terminal_font_is_monospace` (T-043): load the produced `.ttf` with fontTools and assert a single distinct ASCII advance width, `post.isFixedPitch=1`, and family name "Mekzantine Mono". (Runs against the local artifact; the artifact is not committed.)
...
- The banner (T-040) and the terminal-font conversion (T-043) are verified by their
  artifacts (a non-empty PNG; a valid monospace TTF), not by a browser E2E.
