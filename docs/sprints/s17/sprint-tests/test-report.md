...
| [INT-0016](../../../intents/INT-0016-showcase-readme.md) | 4 — reproducible generators; installable mono TTF; no committed font binaries | generators committed + rerun clean; `make-terminal-font.py` → valid monospace "Mekzantine Mono" TTF (installed per-user); `git check-ignore` → no font binary tracked | **pass** |
...
- **Terminal font:** `scripts/make-terminal-font.py` converts the monospace
  MekzantineMono woff2 → a valid TTF (`isFixedPitch`, PANOSE, family "Mekzantine
  Mono") and it is installed per-user; the binary is **not committed** (git-ignored).
