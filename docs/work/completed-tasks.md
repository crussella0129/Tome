...
## T-040 (sprint 17)
- **Description:** Brand banner — the "TOME" wordmark in the Mekzantine display font on the ink-on-parchment palette, as a committed GitHub-safe image
...
## T-044 (sprint 18)
- **Description:** Productionize the Tauri shell — real product name/identifier + Tome branding in `tauri.conf.json`, a bundle icon set generated from the Tome "T" mark, and `tauri:dev` / `tauri:build` launch scripts
- **Intent:** [INT-0017](../intents/INT-0017-tauri-production-port.md)
- **Completed:** 2026-08-16T16:10:00Z
- **Files modified:** src-tauri/tauri.conf.json (`productName` "tome" → "Tome"; `identifier` "com.tome.desktop.spike" → "com.tome.desktop"; `frontendDist: ../dist` and `csp: null` retained), src-tauri/icons/* (regenerated from the Tome "T" mark via `tauri icon electron/assets/icon-light.png` — icon.ico/icon.png/icon.icns + 32/64/128/128@2x PNGs + the Appx Square/StoreLogo set; the emitted android/ and ios/ trees removed as irrelevant to the Windows desktop port), package.json (`tauri:dev` = `npm run build && tauri dev`, `tauri:build` = `npm run build && tauri build`)
- **EARS verified:** `tauri.conf.json` now carries `productName` "Tome", `identifier` `com.tome.desktop`, `frontendDist: ../dist`, `csp: null`; the bundle icon set is the ink-on-parchment Tome "T" (verified by reading back `icons/128x128.png` — the serif "T" on the warm ground, not the default Tauri logo); `npm run tauri:dev` / `npm run tauri:build` exist and chain the frontend build (mirroring the `electron` script). The window title "Tome" and the keyboard-zoom disable are deferred to T-045 (the window is code-built; `app.windows: []`). Full packaging is exercised in T-046.
- **Commit:** `25c03715efa7ac070de6d7abfe9949b975e81cff`

## T-045 (sprint 18)
- **Description:** Theme-aware window icon + zoom hardening in the Tauri shell — Electron parity: the "T" mark swaps with the OS theme, and user zoom (pinch / Ctrl-wheel / Ctrl +/-) is disabled so an accidental gesture can't collapse the reader layout
- **Intent:** [INT-0017](../intents/INT-0017-tauri-production-port.md)
- **Completed:** 2026-08-16T16:35:00Z
- **Files modified:** src-tauri/src/lib.rs (title "Tome"; embedded `window-dark.png`/`window-light.png` via `include_bytes!`; `icon_for(theme)` mirrors `resolveIconVariant` — Dark→dark, else light; `apply_theme_icon` at startup + `WindowEvent::ThemeChanged` swap; `.zoom_hotkeys_enabled(false)` on the builder; Windows `with_webview` → `ICoreWebView2Settings::SetIsZoomControlEnabled(false)`), src-tauri/Cargo.toml (tauri `image-png` feature for runtime PNG decode; Windows-only `webview2-com = "0.38"` — the version Tauri already resolves), src-tauri/Cargo.lock (image/png pulled in by `image-png`), src-tauri/icons/window-{dark,light}.png (new — the two runtime icon variants copied from electron/assets), src-tauri/build.rs + src-tauri/src/main.rs (rustfmt normalized the scaffold's 2-space indent to 4)
- **EARS verified:** compiles clean (`cargo build`), `cargo fmt` applied, `cargo clippy --all-targets` reports **0 warnings**. Title is "Tome"; the theme-icon and zoom-lock code paths are in place. The runtime behaviour (the "T" per OS theme; an accidental pinch/Ctrl-wheel not changing zoom or collapsing the layout) is a visual/behavioural judgment verified at the T-046 launch and surfaced to the user for sign-off (no automated Tauri E2E this sprint — deferred to a WebDriver follow-up).
- **Commit:** `259ec99e39b4c0ce9dc5ea48ed2966930eab0ad4`

## T-046 (sprint 18)
- **Description:** Package the Tauri port into a native Windows installer, verify the built app launches and renders offline with the Tome branding/icon + locked zoom, prove coexistence with the untouched Electron shell + all gates, and update the README
- **Intent:** [INT-0017](../intents/INT-0017-tauri-production-port.md)
- **Completed:** 2026-08-16T17:05:00Z
- **Files modified:** docs/sprints/s18/sprint-tests/port-verification.md (new — installer sizes, launch/screenshot evidence, coexistence table, follow-ups), docs/sprints/s18/sprint-tests/evidence/{tauri-bibliotheca.png,tauri-reader.png} (new — the launched app: Bibliotheca home + a chapter, offline, PrintWindow-captured), README.md (the Tauri section rewritten from "experimental spike" to the native Windows port — `tauri:dev`/`tauri:build`, installer names/sizes, parity note), src-tauri/Cargo.toml (`tauri build` normalized `tauri-build` to `features = []`)
- **EARS verified:** `npm run tauri:build` produced, unsigned, `Tome_0.1.0_x64-setup.exe` (NSIS, 2.0 MB) + `Tome_0.1.0_x64_en-US.msi` (3.1 MB) from an 8.9 MB `app.exe` (criterion 3). The launched app renders the Bibliotheca **and** a chapter (Colophon, via native directory routing) **offline**, with the OS window title "Tome" and the theme-aware "T" titlebar icon (criteria 1, 4 — surfaced to the user for the visual sign-off); zoom is locked via `zoom_hotkeys_enabled(false)` + WebView2 `IsZoomControlEnabled(false)` (criterion 2, behavioural). Coexistence (criterion 4): Vitest 97/97, `astro check` 0 errors, `check:external`/`check:livereload`/`check:multibook`/`check:search` PASS, `check:electron` 6/6, browser `test:e2e` 21/21 — all green; the port touched no web/Electron code. Automated Tauri E2E (WebDriver) is a documented follow-up.
- **Commit:** `5ff9aab696020960068f52024c49a4eec98833d5`

## T-047 (sprint 19)
- **Description:** Build the same `src-tauri/` shell natively on Linux (WebKitGTK) via WSL2 and package it — a reproducible `scripts/build-linux.sh` producing a native `.deb` (+ AppImage best-effort)
- **Intent:** [INT-0018](../intents/INT-0018-tauri-linux-parity.md)
- **Completed:** 2026-08-16T17:15:00Z
- **Files modified:** scripts/build-linux.sh (new — reproducible Linux build: verifies webkit2gtk-4.1, installs the cargo `tauri` CLI, reuses a prebuilt `dist/`, builds `.deb` explicitly then AppImage best-effort so a missing `rpmbuild`/AppImage-tooling can't lose the `.deb`; documents the apt prerequisites + the WSL `CARGO_TARGET_DIR` caveat)
- **EARS verified:** built in WSL2 Ubuntu 26.04 against **webkit2gtk-4.1 2.52.3** (the same `src-tauri/` compiled — `webkit2gtk`/`soup3`/`gtk` crates linked; the `cfg(windows)` `webview2-com` dep correctly excluded). `tauri build` produced **`Tome_0.1.0_amd64.deb`** (4,339,308 B ≈ 4.1 MB; `Package: tome`, `Architecture: amd64`, `Depends: libwebkit2gtk-4.1-0, libgtk-3-0`, ~13 MB installed) and, after installing `xdg-utils`, **`Tome_0.1.0_amd64.AppImage`** (82,647,544 B ≈ 78.8 MB — bundles the GTK/WebKit runtime). Built in the WSL-native fs (reusing the portable Windows-built `dist/`; node not required); binaries not committed (`target/` git-ignored) — path/size recorded. Provisioning (apt webkit stack + xdg-utils) done as user-authorized passwordless-root WSL setup, documented in `build-linux.sh` as requirements, not committed machine state. Minor debt: the `.deb` Maintainer ("you") / Description ("A Tauri App") are scaffold defaults (a metadata-polish follow-up).
- **Commit:** `8fd89437252288c02cab5051b51a8a44db4d8990`

## T-048 (sprint 19)
- **Description:** Launch the built Linux app under WSLg and verify WebKitGTK parity — offline render, "Tome" branding, and zoom that can't collapse the layout; no shell code change was needed
- **Intent:** [INT-0018](../intents/INT-0018-tauri-linux-parity.md)
- **Completed:** 2026-08-16T17:45:00Z
- **Files modified:** docs/sprints/s19/sprint-tests/evidence/{tauri-linux-bibliotheca.png,tauri-linux-reader.png,tauri-linux-zoomtest.png} (new — the launched Linux app captured via X11 `xwd`). No `src-tauri/src/lib.rs` change: WebKitGTK needed no zoom handler (see below).
- **EARS verified:** the built `app` (from T-047) launched under WSLg (`DISPLAY=:0`, with `WEBKIT_DISABLE_DMABUF_RENDERER=1`/`WEBKIT_DISABLE_COMPOSITING_MODE=1`/`LIBGL_ALWAYS_SOFTWARE=1`/`GDK_BACKEND=x11` — the standard WebKitGTK-under-WSL software-render env). It renders **offline** (embedded `dist/`): the Bibliotheca (`tauri-linux-bibliotheca.png`) **and** the Colophon chapter reached by native directory-route navigation (`tauri-linux-reader.png`) — pixel-identical to the Windows/WebView2 render, with the Weston title bar showing **"Tome"** (branding; the title is on the server-side decoration, so the content window's `_NET_WM_NAME` is empty). **Zoom parity:** after 12× Ctrl+scroll-down the capture (`tauri-linux-zoomtest.png`) is **byte-identical** to the pre-gesture reader — WebKitGTK doesn't bind Ctrl+scroll to zoom, so the existing `cfg(not(windows))` no-op + `zoom_hotkeys_enabled(false)` already satisfy the "can't collapse the reader" clause; **no Linux-specific code was required**. External-link routing is the same cross-platform `on_navigation` + `tauri-plugin-opener` path (verified by construction; `xdg-open` is present). Renders/behaviour **surfaced to the user** for sign-off. Notes: the theme-aware window **icon** is best-effort on GTK — under WSLg's Weston frame the window icon isn't surfaced in the title bar (a real desktop uses the packaged `.desktop`/icon-theme "T"); this matches INT-0018's "best-effort on GTK" boundary.
- **Commit:** `e554cd9c590163223f7f52d496c358c3b31ae117`

## T-049 (sprint 19)
- **Description:** Prove the Linux port coexists with the untouched Windows Tauri build + Electron shell + all gates, and extend the README/docs to Windows + Linux
- **Intent:** [INT-0018](../intents/INT-0018-tauri-linux-parity.md)
...
## T-050 (sprint 20)
- **Description:** Center the complete reader stack in the available main-content track and prove wide/mobile stability
- **Intent:** [INT-0019](../intents/INT-0019-centered-reader-search-geometry.md)
- **Completed:** 2026-08-20T04:37:16Z
- **Files modified:** src/layouts/BookLayout.astro (new measure-bounded `.reading-column` around reader search + slotted chapter/pager), e2e/scaling.spec.ts (headed/unheaded wide-centering and hydrated mobile-nav stability tests), docs/SUMMARY.md, docs/intents/INT-0019-centered-reader-search-geometry.md, docs/intents/INT-0020-native-library-folder-management.md, docs/sprints/s20/{sprint-meta.md,sprint-research/research-report.md,sprint-plans/build-plan.md,sprint-plans/test-plan.md,sprint-plans/critique.md}, docs/work/{tasks.md,completed-tasks.md}
- **EARS verified:** `test_scaling_reader_column_centered` proves at 1280/1920/2560 that headed (`/tome/getting-started`) and unheaded (`/tome/about`) pages keep column/prose/pager within the resolved `72ch` measure and centered in `main.content` within 1px, with reader search contained/right-aligned and the rail mode correct. `test_scaling_reader_column_stable_with_mobile_nav` proves at 600px that the hydrated TOC toggles false→true→false without moving the reading-column center by more than 1px or causing overflow. Targeted Playwright: 2/2 passed after formatting. `astro check`: exit 0, 0 errors, 0 warnings, one pre-existing unused-import hint in `scripts/make-banner.mjs`.
- **Commit:** `dc9c8337a73bc60dae464a2e2832ab957a599e98`

## T-051 (sprint 20)
- **Description:** Normalize shared search width and preserve usable open-dialog controls at narrow and breakpoint widths
- **Intent:** [INT-0019](../intents/INT-0019-centered-reader-search-geometry.md)
- **Completed:** 2026-08-20T04:49:09Z
- **Files modified:** src/components/SearchOverlay.module.css (host-filling bounded root, non-compressing trigger affordances, mobile modal stacking, and narrow-field spacing/icon treatment), e2e/scaling.spec.ts (breakpoint matrix plus semantic trigger/dialog geometry assertions across Bibliotheca and reader hosts), src/lib/__tests__/ci-workflow.test.ts (match the already-merged checkout/setup-node v7 workflow pins), scripts/make-banner.mjs (remove an unused import), docs/sprints/s20/sprint-meta.md (record the two resolved gate discoveries), docs/work/{tasks.md,completed-tasks.md}
- **EARS verified:** `test_scaling_search_trigger_usable_across_hosts` proves the shared root and trigger fill each real host up to 26rem, preserve one-line copy, and avoid overflow on both `/` and `/tome/getting-started` across 480, 768/769, and 1023/1024/1040px. `test_scaling_search_dialog_controls_do_not_compress` proves at 320×360 and 480×360 that the focused input remains at least 192px wide, the close control retains its 32×32 CSS target, controls do not overlap, the decorative icon yields only at 320px, the dialog stays in the viewport, and a real close click is not intercepted by the mobile TOC. The complete scaling spec passed 7/7, Vitest passed 97/97, and `astro check` reported 0 errors, 0 warnings, and 0 hints.
- **Commit:** `ee411efca2042f1772627a7db6e83bc20ce5bdc6`
