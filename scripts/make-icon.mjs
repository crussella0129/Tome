// Generates the Tome desktop-app icons: a capital "T" in the display font
// (Mekzantine) on the app's warm ground, rendered with the real woff2 so the
// letterform is exact. Produces BOTH theme variants — `dark` (cream on
// near-black) and `light` (ink on parchment) — each as a multi-size Windows
// `.ico` and a 512px `.png` (Linux window icon), under `electron/assets/`.
//
//   node scripts/make-icon.mjs
//
// The shell picks a variant per the system theme at runtime (see electron/main.cjs).
// Colours mirror the two shipped themes (src/styles/theme.ts).
...
const VARIANTS = {
  dark: { bg: '#16130e', fg: '#e8dcc2' }, // Terminal (dark)
  light: { bg: '#f3e9d2', fg: '#2b2018' }, // Ink on old paper
};
const GLYPH = 'T';
const ICO_SIZES = [256, 128, 64, 48, 32, 16];
const PNG_SIZE = 512;

function pageHtml(size, fontDataUri, { bg, fg }) {
...
async function renderPng(page, size, fontDataUri, palette) {
  await page.setViewportSize({ width: size, height: size });
  await page.setContent(pageHtml(size, fontDataUri, palette), { waitUntil: 'load' });
...
try {
  for (const [name, palette] of Object.entries(VARIANTS)) {
    ...
    for (const size of ICO_SIZES) {
      entries.push({ size, png: await renderPng(page, size, fontDataUri, palette) });
    }
    await writeFile(join(OUT_DIR, `icon-${name}.ico`), buildIco(entries));
    await writeFile(join(OUT_DIR, `icon-${name}.png`), await renderPng(page, PNG_SIZE, fontDataUri, palette));
    console.log(`icon-${name}.ico (${ICO_SIZES.join(',')}) + icon-${name}.png (${PNG_SIZE})`);
  }
  console.log(`→ ${OUT_DIR}`);
} finally {
  await browser.close();
